(ns epiconlineservices.main
  "Kotodama WASM entrypoint for the Epiconlineservices clean-room actor (L4) — Clojure port.
  ...
  facts (`epiconlineservices.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "epiconlineservices")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"epiconlineservices.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "epiconlineservices-compat" :tier tier :entities entities} 200])
...
(defn start! [] :epiconlineservices-compat/ready)
